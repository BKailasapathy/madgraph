...
C     
C     Process: u u~ > uv uv~ WEIGHTED<=2 @1
C     *   Decay: uv > p1 c WEIGHTED<=1
C     Process: c c~ > uv uv~ WEIGHTED<=2 @1
C     *   Decay: uv > p1 c WEIGHTED<=1
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
...
C     Needed for v4 models
      ...
      DOUBLE PRECISION FK_MDL_WUV
      DOUBLE PRECISION FK_ZERO
      DOUBLE PRECISION FK_MDL_WPE2
      DOUBLE PRECISION FK_MDL_WPE1
      SAVE FK_MDL_WUV
      SAVE FK_ZERO
      SAVE FK_MDL_WPE2
      SAVE FK_MDL_WPE1
...
C     ----------
      IF (FIRST) THEN
        FIRST=.FALSE.
        FK_MDL_WUV = SIGN(MAX(ABS(MDL_WUV), ABS(MDL_MUV
     $   *SMALL_WIDTH_TREATMENT)), MDL_WUV)
        FK_MDL_WPE1 = SIGN(MAX(ABS(MDL_WPE1), ABS(MDL_MPE1
     $   *SMALL_WIDTH_TREATMENT)), MDL_WPE1)
        FK_MDL_WPE2 = SIGN(MAX(ABS(MDL_WPE2), ABS(MDL_MPE2
     $   *SMALL_WIDTH_TREATMENT)), MDL_WPE2)
     ...
     $   , ZERO)
      ...
      CALL FFS1_1(W(1,4),W(1,3),GC_71,MDL_MUV, FK_MDL_WUV,W(1,5))
      CALL IXXXXX(P(0,5),MDL_MUV,NHEL(5),-1*IC(5),W(1,4))
      CALL FFV1P0_3(W(1,1),W(1,2),GC_11,ZERO, FK_ZERO,W(1,3))
C     Amplitude(s) for diagram number 1
      CALL FFV1_0(W(1,4),W(1,5),W(1,3),GC_11,AMP(1))
      CALL FFS2_3(W(1,1),W(1,5),GC_72,MDL_MPE2, FK_MDL_WPE2,W(1,3))
C     Amplitude(s) for diagram number 2
      CALL FFS1_0(W(1,4),W(1,2),W(1,3),GC_72,AMP(2))
      CALL FFS2_3(W(1,1),W(1,5),GC_71,MDL_MPE1, FK_MDL_WPE1,W(1,3))
C     Amplitude(s) for diagram number 3
      CALL FFS1_0(W(1,4),W(1,2),W(1,3),GC_71,AMP(3))
C     JAMPs contributing to orders ALL_ORDERS=1
      JAMP(1,1)=+1D0/2D0*(+1D0/3D0*AMP(1))
      JAMP(2,1)=-1D0/2D0*AMP(1)+AMP(2)+AMP(3)
