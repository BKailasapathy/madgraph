...
class user : public AnalyzerBase
{
 ...
 private : 
  ...
  std::vector<const MCParticleFormat*> _P_h_I1I_PTorderingfinalstate_REG_;
  std::vector<const MCParticleFormat*> _P_hPTorderingfinalstate_REG_;
  bool isP__hPTorderingfinalstate(const MCParticleFormat* part) const {
     if ( part==0 ) return false;
     if ( !PHYSICS->Id->IsFinalState(part) ) return false;
     if ( (part->pdgid()!=25) ) return false;
     return true; }
};
}
